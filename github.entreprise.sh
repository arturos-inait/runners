...
#--------------------------------------
# Get a config token for Enterprise
#--------------------------------------
echo
...
# The endpoint for generating an enterprise-level runner registration token
export RUNNER_TOKEN=$(curl -s -X POST ${base_api_url}/enterprises/${enterprise_name}/actions/runners/registration-token -H "accept: application/vnd.github.v3+json" -H "authorization: token ${RUNNER_CFG_PAT}" | jq -r '.token')
